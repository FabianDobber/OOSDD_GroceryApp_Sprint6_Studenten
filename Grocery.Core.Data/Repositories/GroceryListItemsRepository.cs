...
namespace Grocery.Core.Data.Repositories
{
    public class GroceryListItemsRepository : DatabaseConnection, IGroceryListItemsRepository
    {
            ...
            using (SqliteCommand command = new(selectQuery, Connection))
            {
                ...
                using (SqliteDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        int productId = reader.GetInt32(2);
                        int amount = reader.GetInt32(3);
                        itemsOnList.Add(new GroceryListItem(id, gId, productId, amount));
                    }
                }
            }
            CloseConnection();
        ...
        {
            string insertQuery = "INSERT INTO GroceryListItem(GroceryListId, ProductId, Amount) VALUES(@GroceryListId, @ProductId, @Amount) Returning Id;";
            OpenConnection();
        ...
        {
            string updateQuery = "UPDATE GroceryListItem SET ProductId = @ProductId, Amount = @Amount WHERE Id = @Id";
            OpenConnection();
